...
func (s *SessionInternalAPIServiceService) PublishOnConnectEvent(ctx context.Context, event *OnConnectEvent) error {
	return s.publisher.PublishOnConnectEvent(ctx, event)
}
func (s *SessionInternalAPIServiceService) PublishOnDisconnectEvent(ctx context.Context, event *OnDisconnectEvent) error {
	return s.publisher.PublishOnDisconnectEvent(ctx, event)
}
...
func (s *SessionInternalAPIServiceEventsPublisher) PublishOnConnectEvent(ctx context.Context, event *OnConnectEvent) error {
	var r []byte
	var err error
	if event != nil {
		r, err = event.Marshal()
	...
	cb := func(ctx context.Context) error {
		...
		err = s.ps.Publish("SessionInternalAPIService", "OnConnectEvent", ctxData, r)
		if err != nil {
	...
	return s.wrapper(ctx, "SessionInternalAPIService", "OnConnectEvent", cb)
}
func (s *SessionInternalAPIServiceEventsPublisher) PublishOnDisconnectEvent(ctx context.Context, event *OnDisconnectEvent) error {
	var r []byte
	var err error
	if event != nil {
		r, err = event.Marshal()
	...
	cb := func(ctx context.Context) error {
		...
		err = s.ps.Publish("SessionInternalAPIService", "OnDisconnectEvent", ctxData, r)
		if err != nil {
			return fmt.Errorf("error while publishing error: %w", err)
		}
	...
	return s.wrapper(ctx, "SessionInternalAPIService", "OnDisconnectEvent", cb)
}
...
func (c *SessionInternalAPIServiceClient) SubscribeOnConnectEvent(callback func(ctx context.Context, event *OnConnectEvent) error) (stop pubsub.CancelFunc, err error) {
	stop, err = c.client.GetConfig().PubSub.Subscribe("SessionInternalAPIService", "OnConnectEvent", func(ctxData, event []byte) error {
		log := c.client.GetConfig().Logger
		ev := OnConnectEvent{}
		err := ev.Unmarshal(event)
		log.Debug("got event", "SessionInternalAPIService", "OnConnectEvent")
		if err != nil {
			log.Error(err, "error while unmarshalling event", "SessionInternalAPIService", "OnConnectEvent", string(event))
			return fmt.Errorf("error while unmarshalling event: %w", err)
		}
		ctx, _, err := c.client.GetConfig().ContextMarshaller.Unmarshal(ctxData)
		if err != nil {
			log.Error(err, "error while unmarshalling context data", "SessionInternalAPIService", "OnConnectEvent", string(ctxData))
		...
		if wrap != nil {
			...
			err = wrap(ctx, "SessionInternalAPIService", "OnConnectEvent", wrapCB)
			return err
		}
		err = callback(ctx, &ev)
		if err != nil {
...
func (c *SessionInternalAPIServiceClient) SubscribeOnDisconnectEvent(callback func(ctx context.Context, event *OnDisconnectEvent) error) (stop pubsub.CancelFunc, err error) {
	stop, err = c.client.GetConfig().PubSub.Subscribe("SessionInternalAPIService", "OnDisconnectEvent", func(ctxData, event []byte) error {
		log := c.client.GetConfig().Logger
		ev := OnDisconnectEvent{}
		err := ev.Unmarshal(event)
		log.Debug("got event", "SessionInternalAPIService", "OnDisconnectEvent")
		if err != nil {
			log.Error(err, "error while unmarshalling event", "SessionInternalAPIService", "OnDisconnectEvent", string(event))
			return fmt.Errorf("error while unmarshalling event: %w", err)
		}
		ctx, _, err := c.client.GetConfig().ContextMarshaller.Unmarshal(ctxData)
		if err != nil {
			log.Error(err, "error while unmarshalling context data", "SessionInternalAPIService", "OnDisconnectEvent", string(ctxData))
			return fmt.Errorf("error while unmarshalling context data: %w", err)
		}
		wrap := c.client.GetConfig().PubSubWrapper
		if wrap != nil {
			...
			err = wrap(ctx, "SessionInternalAPIService", "OnDisconnectEvent", wrapCB)
			return err
		}
		err = callback(ctx, &ev)
		if err != nil {
