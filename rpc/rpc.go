...
type Caller interface {
	Call(namespace string, functionName string, context []byte, arguments []byte) (response []byte, err error)
}

type Listener interface {
	//TODO: listen may halt in progress. think about it
	Listen(namespace string, onListen func(functionName string, context []byte, arguments []byte) (response []byte, err error)) context.CancelFunc
}
