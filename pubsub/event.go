...
type Publisher interface {
	Publish(namespace string, eventName string, ctx, event []byte) error
}

type CancelFunc func()

type Subscriber interface {
	Subscribe(namespace string, eventName string, callback func(ctx, event []byte) error) (CancelFunc, error)
